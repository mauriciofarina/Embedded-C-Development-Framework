 ...
 * @brief LittleFS FileSystem Library AT45DB641E Device Implementation
 */
#ifdef USE_LITTLEFS
#ifdef USE_DEVICE_AT45DB641E
#include "filesystem/littlefs/lib_filesystem_littlefs_private.h"

#include "at45db641e/dev_at45db641e.h"
...
static at45db641e_t handler; /** Device Handler */

#ifdef USE_RTOS_KERNEL
static rtos_binary_semaphore_t transfer_completed = {0}; /** Transfer Completed Semaphore */
#else
static bool transfer_completed = false; /** Transfer Completed Flag */
#endif
...
void memory_device_init(void) {
    ...
    dev_at45db641e_init(&handler, &init);

    assert(dev_at45db641e_is_device_ready(&handler));
    assert(dev_at45db641e_set_page_size(&handler));
    assert(dev_at45db641e_is_device_ready(&handler));
    assert(dev_at45db641e_has_valid_device_id(&handler));
}
...
int littlefs_read(const struct lfs_config *c, lfs_block_t block, lfs_off_t off, void *buffer, lfs_size_t size) {
    ...
    if (dev_at45db641e_is_device_ready(&handler)) {
        if (dev_at45db641e_read_page(&handler, buffer, block)) {
            error = LFS_ERR_OK;
        }
    }
...
int littlefs_prog(const struct lfs_config *c, lfs_block_t block, lfs_off_t off, const void *buffer, lfs_size_t size) {
    ...
    if (dev_at45db641e_is_device_ready(&handler)) {
        if (dev_at45db641e_write_page(&handler, (void *)buffer, block)) {
            error = LFS_ERR_OK;
        }
    }
    return error;
...
int littlefs_erase(const struct lfs_config *c, lfs_block_t block) {
    ...
    if (dev_at45db641e_is_device_ready(&handler)) {
        if (dev_at45db641e_erase_page(&handler, block)) {
            error = LFS_ERR_OK;
        }
    }
